...
def load_featrues_path(file_path):
    pattern = re.compile(r"^(.+?)_(\w+)_(\d+)(?:_features)?\.csv$", re.IGNORECASE)
    fname = os.path.basename(file_path)
    m = pattern.match(fname)
    if not m:
        raise ValueError(f"Archivo no válido: {fname}")
    ...
    try:
        ...
        raise ValueError(f"No se pudo cargar {fname}: {e}")
...
def plot_lda_results(X_lda, y_enc, var_ratio, importancias, idx_ord,
    ...
    try:
        var_ratio = np.asarray(var_ratio, dtype=float).ravel()
        if var_ratio.size == 0:
            raise ValueError("var_ratio vacío")

        n_plot = min(n_comp, var_ratio.size)
        var_ratio_plot = var_ratio[:n_plot]
        comp_labels = [f"LD{i+1}" for i in range(n_plot)]
    except Exception as exc:
        print(f"    ⚠️ No se pudo generar la gráfica LDA para {nombre_set}: {exc}")
        return
    ...
    bars        = ax1.bar(comp_labels, var_ratio_plot * 100,
                          color=PALETTE[:n_plot], edgecolor="white")
    ax1.plot(comp_labels, np.cumsum(var_ratio_plot) * 100,
             "o--", color="black", linewidth=1.5, markersize=5, label="Acumulada")
    ax1.set_ylabel("Varianza explicada (%)")
    ax1.set_title("Varianza por componente")
    ax1.set_ylim(0, 115)
    ax1.legend(fontsize=9)
    for bar, val in zip(bars, var_ratio_plot * 100):
    ...
    top_n    = min(10, n_plot, len(importancias))
...
def procesar_lda_caracteristicas(
    df,
    usuario,
    tipo_clase="Muestra",
    label_col="label",
    ...
    """Optimiza LDA sobre un DataFrame de características en memoria.

    El DataFrame debe tener una columna de etiquetas. La función evalúa los grupos de
    features disponibles, genera resultados LDA, guarda CSV y gráficas si se provee
    un output_path.
    """
    if not isinstance(df, pd.DataFrame):
        raise ValueError("Se requiere un DataFrame de características.")
    if label_col not in df.columns:
        raise ValueError(f"No se encontró la columna de etiquetas '{label_col}' en el DataFrame.")

    data_total = df.copy()
    data_total[label_col] = data_total[label_col].astype(str)
    feat_cols = [c for c in data_total.columns if c != label_col]
    if not feat_cols:
        raise ValueError("No se encontraron columnas de características en el DataFrame.")

    etiquetas = sorted(data_total[label_col].unique().tolist())
    n_cls = len(etiquetas)
    if n_cls < 2:
        raise ValueError("Se necesitan al menos 2 clases distintas para aplicar LDA.")

    le = LabelEncoder()
    y_enc = le.fit_transform(data_total[label_col].values)

    all_feature_sets = get_feature_sets(feat_cols)
    if grupos_filter:
        all_feature_sets = {k: v for k, v in all_feature_sets.items() if k in grupos_filter}
    if not all_feature_sets:
        raise ValueError("No se encontraron grupos de features válidos con el filtro proporcionado.")

    if output_path is None:
        output_path = os.path.join(".", usuario, "LDA_Caracteristicas")
    os.makedirs(output_path, exist_ok=True)

    print(f"\n  {'─'*56}")
    print(f"  📊 LDA sobre características en memoria — {usuario} / {tipo_clase}")
    print(f"  {'─'*56}")
    print(f"  Grupos a evaluar: {list(all_feature_sets.keys())}")
    print(f"  Total épocas: {len(data_total)} | Clases: {etiquetas}\n")
    ...
    for nombre_set, features in all_feature_sets.items():
        ...
        if save_output:
            group_dir = os.path.join(output_path, nombre_set)
            os.makedirs(group_dir, exist_ok=True)
            out_df = pd.DataFrame(X_lda, columns=[f"LD{i+1}" for i in range(X_lda.shape[1])])
            out_df[label_col] = data_total[label_col].values
            out_csv = os.path.join(group_dir, f"{usuario}_{tipo_clase}_{nombre_set}_lda.csv")
            out_df.to_csv(out_csv, index=False)
            print(f"    💾 LDA CSV: {out_csv}")

        plot_lda_results(X_lda, y_enc, var_ratio, importancias, idx_ord,
                         comp_names, nombre_set, output_path,
                         tipo_clase, le, usuario)

    if resumen:
        print(f"\n  {'='*56}")
        print(f"  RESUMEN — {usuario} / {tipo_clase}")
        print(f"  {'='*56}")
        print(pd.DataFrame(resumen).to_string(index=False))

        save_resumen_csv(resumen, output_path)
        plot_resumen_cv(resumen, output_path, tipo_clase, usuario)

        validos = [r for r in resumen if not np.isnan(r["cv_acc"])]
        if validos:
            mejor = max(validos, key=lambda r: r["cv_acc"])
            print(f"\n  🏆 Mejor grupo: {mejor['grupo']}  (CV Acc = {mejor['cv_acc']:.4f} ± {mejor['cv_std']:.4f})")

    print(f"\n  ✅ LDA en memoria guardado en: {output_path}\n")
    return resumen
