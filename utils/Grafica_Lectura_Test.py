...
def graficar_captura_sobrepuesta(user, letter, capture_number):
    """
    ...
    try:
        ...
        for idx, column in enumerate(channels):
            plt.plot(df['Tm'], df[column], linewidth=0.8, color=color_array[idx], label=column)
...
while True:  # 🔄 Bucle principal para el cambio de usuario
    ...
    while True:
        ...
        try:
            ...
            # Graficar la captura
            if graficar_captura_sobrepuesta(user, letra, capture_number):
