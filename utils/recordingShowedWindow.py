...
class RecordingShowedWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setup_ui()
        self.load_users()
    ...
    def setup_ui(self):
        ...
        #self.combo_box_users.addItems(['User1', 'User2', 'User3'])  # Placeholder users, will be loaded dynamically
    ...
    def graficar_captura(self,user, letter, capture_number, apply_notch=False, clear_baseline=False,file_path=None):
        ...
        try:
            ...
            fig.suptitle(f'Señales EEG - {user} {letter} #{capture_number} { "con notch" if apply_notch else ""} {"sin linea base" if clear_baseline else ""}', fontsize=10)
